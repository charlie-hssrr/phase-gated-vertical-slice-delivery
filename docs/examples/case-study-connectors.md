...
- workspace + ownership model (`ownerKey`, `workspaceId`)
- ingestion pipeline (files → indexing → retrieval)
- cloud-based storage and search infrastructure
...
- ownership model (`ownerKey`, `workspaceId`)
...
- No OAuth tokens stored in the core database
